...
test('smoke test exercises every durable user domain',()=>{
 for(const method of ['setWatchlist','setNotificationPreferences','markNotificationsRead','registerNotificationDevice','createPaperTrade','getSnapshot','listNotificationDevices','listPaperTrades','closePaperTrade'])assert.match(smoke,new RegExp(method));
});

test('smoke test always cleans synthetic data',()=>{
 assert.match(smoke,/finally/);
 for(const table of ['watchlist_items','notification_preferences','notification_reads','notification_devices','paper_trades','user_profiles'])assert.match(smoke,new RegExp(`delete from ${table}`));
