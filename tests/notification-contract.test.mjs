...
test('manifest publishes notification and storage contract v2.3',()=>{assert.match(manifest,/schemaVersion:'2\.3\.0'/);for(const endpoint of ['/api/user-sync/health','/api/user-sync/notification-devices','/api/user-sync/decision-notes','/api/notifications/personalized','/api/notifications/dispatch-preview'])assert.match(manifest,new RegExp(endpoint.replaceAll('/','\\/')));assert.match(manifest,/notificationDevices/);assert.match(manifest,/decisionNotes/);assert.match(manifest,/timeZoneAware/);assert.match(manifest,/healthModule/);assert.match(manifest,/dry-run-until-provider-delivery/);});
