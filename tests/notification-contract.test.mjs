...
const delivery=fs.readFileSync('lib/push-delivery.ts','utf8');
const mobileDelivery=fs.readFileSync('lib/mobile-push-delivery.ts','utf8');
const realDispatchRoute=fs.readFileSync('app/api/notifications/dispatch/route.ts','utf8');
...
const deliveryMigration=fs.readFileSync('db/migrations/0007_notification_delivery_log.sql','utf8');
...
test('provider delivery preserves policy and durable deduplication',()=>{assert.match(delivery,/buildPushDispatchPlan/);assert.match(delivery,/deliveryRecorded/);assert.match(delivery,/recordDelivery/);assert.match(mobileDelivery,/sendFcm/);assert.match(mobileDelivery,/sendApns/);assert.match(deliveryMigration,/notification_deliveries/);assert.match(realDispatchRoute,/NOTIFICATION_DISPATCH_SECRET/);});
...
test('manifest publishes notification and storage contract v2.5',()=>{assert.match(manifest,/schemaVersion:'2\.5\.0'/);for(const endpoint of ['/api/user-sync/health','/api/user-sync/notification-devices','/api/user-sync/decision-notes','/api/user-sync/workspace','/api/notifications/personalized','/api/notifications/dispatch-preview','/api/notifications/dispatch'])assert.match(manifest,new RegExp(endpoint.replaceAll('/','\\/')));assert.match(manifest,/notificationDevices/);assert.match(manifest,/notificationDeliveries/);assert.match(manifest,/decisionNotes/);assert.match(manifest,/userWorkspace/);assert.match(manifest,/timeZoneAware/);assert.match(manifest,/healthModule/);assert.match(manifest,/provider-delivery-when-configured/);});
