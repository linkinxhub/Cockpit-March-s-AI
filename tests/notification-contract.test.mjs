...
test('manifest publishes notification and storage contract v2.6',()=>{assert.match(manifest,/schemaVersion:'2\.6\.0'/);for(const endpoint of ['/api/user-sync/health','/api/user-sync/notification-devices','/api/user-sync/decision-notes','/api/user-sync/workspace','/api/notifications/personalized','/api/notifications/dispatch-preview','/api/notifications/dispatch'])assert.match(manifest,new RegExp(endpoint.replaceAll('/','\\/')));assert.match(manifest,/notificationDevices/);assert.match(manifest,/notificationDeliveries/);assert.match(manifest,/decisionNotes/);assert.match(manifest,/userWorkspace/);assert.match(manifest,/timeZoneAware/);assert.match(manifest,/healthModule/);assert.match(manifest,/provider-delivery-when-configured/);});
