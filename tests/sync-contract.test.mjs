...
test('identity contract is shared and Flutter never invents a user id',()=>{assert.match(userIdentity,/oai-authenticated-user-email/);assert.match(meRoute,/getSharedUserIdentity/);assert.match(flutterIdentity,/\/api\/me/);assert.doesNotMatch(flutterIdentity,/random|uuid|deviceId/i);});
test('durable user domains are schema-backed and never silently memory-backed',()=>{for(const table of ['user_profiles','watchlist_items','notification_preferences','notification_reads','paper_trades'])assert.match(userSyncSchema,new RegExp(table));assert.match(userSyncStore,/UserSyncStore/);assert.match(userSyncStore,/USER_SYNC_STORE/);assert.doesNotMatch(userSyncStore,/new Map|localStorage|sessionStorage/);assert.match(userSyncCapabilitiesRoute,/userSyncStoreConfigured/);assert.match(flutterUserSync,/\/api\/user-sync\/capabilities/);});
test('sync manifest publishes contract version and endpoints',()=>{assert.match(manifest,/schemaVersion:'1\.5\.0'/);for(const endpoint of ['/api/me','/api/user-sync/capabilities','/api/scanner','/api/history','/api/indicators','/api/ichimoku-history','/api/market-status','/api/news','/api/news-alerts'])assert.match(manifest,new RegExp(endpoint.replaceAll('/','\\/')));});
