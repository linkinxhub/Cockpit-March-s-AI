...
export default function OwnerSettingsShortcuts(){
 ...
 return <section style={{border:'1px solid #39574b',borderRadius:16,padding:20,margin:'20px 0',background:'#0d202b'}} aria-label={c[0]}><h3 style={{color:'#bafa70',marginBottom:8}}>{c[0]}</h3><p>{c[1]}</p><div style={{display:'flex',flexWrap:'wrap',gap:12,margin:'16px 0'}}>{[[c[2],c[3],'/admin/ai-settings'],[c[4],c[5],'/admin/stripe-settings'],[{fr:'Pages & identité SMARTDEV',en:'Pages & SMARTDEV identity',de:'Seiten & SMARTDEV-Identität',nl:'Pagina’s & SMARTDEV-identiteit'}[lang],{fr:'À propos, confidentialité et rédaction IA',en:'About, privacy and AI writing',de:'Über uns, Datenschutz und KI-Texte',nl:'Over ons, privacy en AI-teksten'}[lang],'/admin/public-content']].map(([title,description,url])=><a key={url} href={url} style={{flex:'1 1 220px',display:'block',padding:16,border:'1px solid #476854',borderRadius:12,background:'#17323b',color:'#e5f3ee',textDecoration:'none'}}><strong style={{display:'block',marginBottom:6}}>{title} →</strong><span style={{fontSize:14,color:'#bbcecb'}}>{description}</span></a>)}</div><p style={{fontSize:14,color:'#b6c8c4'}}>{c[6]}</p></section>;
}
