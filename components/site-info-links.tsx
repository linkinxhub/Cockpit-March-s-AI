...
export default function SiteInfoLinks(){const [lang]=useLanguage(),c=infoLabels[lang];return <footer className="site-info-links"><a className="smartdev-credit" href="/qui-sommes-nous">{c[0]} <span>↗</span></a><nav aria-label={c[6]}>{pageKeys.map((p,i)=><a key={p} href={paths[p]}>{c[i+1]}</a>)}<a href="/contact">{{fr:"Contactez-nous",en:"Contact us",de:"Kontakt",nl:"Contact"}[lang]}</a></nav></footer>;}
