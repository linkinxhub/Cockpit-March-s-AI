...
type Snapshot={readNotificationIds?:string[]};

export default function NotificationsPage(){
 const[events,setEvents]=useState<Event[]>([]),[severity,setSeverity]=useState('IMPORTANT'),[loading,setLoading]=useState(true),[read,setRead]=useState<Set<string>>(new Set()),[syncError,setSyncError]=useState('');
 useEffect(()=>{fetch('/api/user-sync/snapshot',{cache:'no-store'}).then(async r=>{const d=await r.json();if(!r.ok)throw new Error(d.error||`sync_${r.status}`);return d;}).then(d=>{const snapshot=(d.snapshot||{}) as Snapshot;setRead(new Set(Array.isArray(snapshot.readNotificationIds)?snapshot.readNotificationIds:[]));}).catch(e=>setSyncError(String(e)));},[]);
