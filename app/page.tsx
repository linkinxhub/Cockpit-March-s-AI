...
export default function Home() {
  ...
  const [ichimokuLayers, setIchimokuLayers] = useState({ price:true, cloud:true, lines:true, signals:true });
  ...
  const technicalStudy = useMemo(() => {
    ...
    const midpoint = (end:number, period:number) => {
      ...
      ichimokuData = chart.map((point,index) => {
        ...
        const chikou = chart[index + 26]?.price ?? null;
        return { ...point, tenkan, kijun, spanA, spanB, chikou, regime:bullish ? "ACHAT" : bearish ? "VENTE" : "ATTENDRE" };
      }),
    ...
    const signalByTime = new Map(signals.map((signal) => [signal.t, signal])),
      visibleData = ichimokuData.slice(-80).map((point) => {
        const signal = signalByTime.get(point.t);
        return {
          ...point,
          action:signal?.action,
          reasons:signal?.reasons,
          outcome:signal?.outcome,
          buySignalPrice:signal?.action === "ACHAT" ? point.price : null,
          sellSignalPrice:signal?.action === "VENTE" ? point.price : null,
              ...
              <div className="ichimokuFocus">
                ...
                <div className="ichimokuLayout">
                  <div className="ichimokuChart">
                    <div className="mixedChartHead">
                      <div><small>GRAPHIQUE MIXTE</small><b>{active.symbol} + Ichimoku · {timeframeLabel}</b></div>
                      <div className="layerControls" aria-label="Éléments visibles du graphique">
