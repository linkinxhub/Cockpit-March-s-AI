...
const expectedBranch='ops/apply-user-workspace-0005';
...
async function execute(req:Request){
 ...
 try{
  ...
  return Response.json({ok:true,changed:result.changed,healthy:result.after.missingTables.length===0&&result.after.missingPreferenceColumns.length===0,migrations:result.after.migrations},{headers:{'Cache-Control':'no-store','X-Robots-Tag':'noindex'}});
