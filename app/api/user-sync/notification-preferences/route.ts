...
export async function PUT(req:Request){
 const auth=await authorizeFeatureApi('ALERTS');if(auth.response)return auth.response;const user=auth.context.identity;if(!userSyncStoreConfigured())return Response.json({error:'storage_not_configured'},{status:503});
