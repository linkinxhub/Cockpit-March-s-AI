...
export async function PUT(req:Request){
 const access=await authorizeFeatureApi('ALERTS');if(access.response)return access.response;const user=access.context.identity;
 if(!userSyncStoreConfigured())return Response.json({error:'storage_not_configured'},{status:503});
 let body:any;try{body=await readBoundedJson(req,16_000);}catch(e){return Response.json({error:e instanceof RequestSecurityError?e.message:'invalid_json'},{status:e instanceof RequestSecurityError?e.status:400});}
 ...
 const token=typeof body?.token==='string'&&body.token.trim()?body.token.trim().slice(0,4096):null,endpoint=typeof body?.endpoint==='string'&&body.endpoint.trim()?body.endpoint.trim().slice(0,4096):null,p256dh=typeof body?.p256dh==='string'&&body.p256dh.trim()?body.p256dh.trim().slice(0,1024):null,auth=typeof body?.auth==='string'&&body.auth.trim()?body.auth.trim().slice(0,1024):null;
 ...
 const id=typeof body?.id==='string'&&body.id.trim()?body.id.trim().slice(0,160):randomUUID();
