...
class _NotificationPreferencesViewState extends State<NotificationPreferencesView>{
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Préférences News')),body:FutureBuilder<UserSyncSnapshot>(future:future,builder:(context,s){
    ...
    return ListView(padding:const EdgeInsets.all(16),children:[
      ...
      DropdownButtonFormField<String>(initialValue:p.minimumSeverity,decoration:const InputDecoration(labelText:'Seuil minimum'),items:['INFO','IMPORTANT','CRITIQUE'].map((v)=>DropdownMenuItem(value:v,child:Text(v))).toList(),onChanged:(v){if(v!=null)setState(()=>prefs=p.copyWith(minimumSeverity:v));}),
