...
const aad=encoder.encode('cockpit-stripe-settings/v1');
...
async function key(secret:string){if(secret.length<32)throw Error('encryption_unavailable');const hash=await crypto.subtle.digest('SHA-256',encoder.encode('cockpit-stripe-settings/v1:'+secret));return crypto.subtle.importKey('raw',hash,{name:'AES-GCM'},false,['encrypt','decrypt']);}
