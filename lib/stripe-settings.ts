...
export async function getStripeSecrets():Promise<StripeSecrets>{const stored=await readStripeSettings();if(stored){const value=JSON.parse(await openKey(stored.encrypted_key,secret()));if(!isTestKey(value.apiKey)||!isWebhookSecret(value.webhookSecret))throw Error('invalid_saved_configuration');return {...value,...(isWebhookSecret(process.env.STRIPE_MANAGED_WEBHOOK_SECRET||'')?{webhookSecret:process.env.STRIPE_MANAGED_WEBHOOK_SECRET}: {})};}
 return {apiKey:process.env.STRIPE_RESTRICTED_KEY?.trim()||'',webhookSecret:process.env.STRIPE_MANAGED_WEBHOOK_SECRET?.trim()||process.env.STRIPE_WEBHOOK_SECRET?.trim()||''};}
export async function stripeSettingsStatus(){const c=await getStripeSecrets();return {keyConfigured:isTestKey(c.apiKey),webhookConfigured:isWebhookSecret(c.webhookSecret),configured:isTestKey(c.apiKey)&&isWebhookSecret(c.webhookSecret),canSave:stripeCanSave(),mode:'test',managedWebhook:isWebhookSecret(process.env.STRIPE_MANAGED_WEBHOOK_SECRET||'')};}
