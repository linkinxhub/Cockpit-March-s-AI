import {aiPlanLimit} from './ai-plan-policy';
...
export const PLAN_ENTITLEMENTS:Record<CommercialPlan,PlanDefinition>={
 DISCOVERY:{features:['DASHBOARD_BASIC','TECHNICAL_INDICATORS','AI_INSTANT_ANALYSIS'],limits:{ALL_ASSETS:{maximum:12},FULL_TIMEFRAMES:{maximum:3},ALERTS:{maximum:0},AI_INSTANT_ANALYSIS:{monthly:5}}},
 PRO:{features:['AI_INSTANT_ANALYSIS','DASHBOARD_BASIC','ALL_ASSETS','REALTIME_REFRESH','FULL_TIMEFRAMES','TECHNICAL_INDICATORS','ICHIMOKU_ADVANCED','FORECASTS','MULTI_TIMEFRAME','ALERTS','EXTENDED_HISTORY'],limits:{ALERTS:{maximum:25},AI_INSTANT_ANALYSIS:{monthly:100}}},
...
export function canAccess(feature:Feature,membership:AccountMembership):boolean{return membership.accountStatus!=='SUSPENDED'&&(membership.role==='ADMIN'||PLAN_ENTITLEMENTS[effectivePlan(membership)].features.includes(feature));}
...
export function featureLimit(feature:Feature,membership:AccountMembership):FeatureLimit|null{if(feature==='AI_INSTANT_ANALYSIS')return {monthly:aiPlanLimit(membership.plan,membership.subscriptionStatus,membership.role,membership.accountStatus)??undefined};if(membership.role==='ADMIN')return null;return PLAN_ENTITLEMENTS[effectivePlan(membership)].limits[feature]||null;}
