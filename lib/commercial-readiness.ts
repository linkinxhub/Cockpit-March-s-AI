...
export async function commercialReadiness(){
 const blockers:string[]=[],warnings:string[]=[];const url=dbUrl();let billingSchema=false,pushDeliverySchema=false;
 if(!url)blockers.push('database_not_configured');else try{const sql=neon(url);const rows=await sql`select table_name from information_schema.tables where table_schema='public' and table_name in ('billing_subscriptions','billing_webhook_events','notification_deliveries')`;const names=new Set(rows.map((r:any)=>String(r.table_name)));billingSchema=names.has('billing_subscriptions')&&names.has('billing_webhook_events');pushDeliverySchema=names.has('notification_deliveries');if(!billingSchema)blockers.push('billing_migration_0006_missing');if(!pushDeliverySchema)blockers.push('notification_delivery_migration_0007_missing');}catch{blockers.push('database_health_failed');}
 ...
 if(!present('NOTIFICATION_DISPATCH_SECRET'))warnings.push('notification_dispatch_scheduler_not_configured');
 ...
 const webPush=present('WEB_PUSH_PUBLIC_KEY')&&present('WEB_PUSH_PRIVATE_KEY');const fcm=present('FCM_PROJECT_ID')&&present('FCM_CLIENT_EMAIL')&&present('FCM_PRIVATE_KEY');const apns=present('APNS_KEY_ID')&&present('APNS_TEAM_ID')&&present('APNS_PRIVATE_KEY')&&present('APNS_BUNDLE_ID');if(!webPush)warnings.push('web_push_delivery_not_configured');if(!fcm)warnings.push('android_push_delivery_not_configured');if(!apns)warnings.push('ios_push_delivery_not_configured');
 if(!present('NEXT_PUBLIC_SUPPORT_EMAIL'))blockers.push('support_email_missing');
 return{ready:blockers.length===0&&stripeMode==='live',billingSchema,pushDeliverySchema,stripeMode,push:{web:webPush,android:fcm,ios:apns},blockers,warnings,checkedAt:new Date().toISOString()};
}
