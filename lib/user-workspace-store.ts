...
function sanitizeAlerts(value:unknown):PriceAlertState[]{if(!Array.isArray(value))return[];return value.slice(0,100).flatMap((item:any)=>{const id=Number(item?.id),price=String(item?.price??'').trim().slice(0,40),symbol=String(item?.symbol??'').trim().slice(0,32);return Number.isFinite(id)&&symbol&&price?[{id,symbol,price}]:[];});}
function boundedPassport(item:unknown):PassportState|null{if(!item||typeof item!=='object'||Array.isArray(item))return null;try{const text=JSON.stringify(item);if(text.length>8_000)return null;const parsed=JSON.parse(text);return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:null;}catch{return null;}}
function sanitizePassports(value:unknown):PassportState[]{if(!Array.isArray(value))return[];return value.slice(0,100).flatMap(item=>{const clean=boundedPassport(item);return clean?[clean]:[];});}
