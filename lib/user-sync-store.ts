...
function postgresStore():UserSyncStore{
 ...
 return{
  ...
  async registerNotificationDevice(userId,value){const now=Date.now();const rows=await sql`insert into notification_devices(id,user_id,platform,provider,token,endpoint,p256dh,auth,created_at,updated_at) values(${value.id},${userId},${value.platform},${value.provider},${value.token},${value.endpoint},${value.p256dh},${value.auth},${now},${now}) on conflict(id) do update set user_id=excluded.user_id,platform=excluded.platform,provider=excluded.provider,token=excluded.token,endpoint=excluded.endpoint,p256dh=excluded.p256dh,auth=excluded.auth,updated_at=excluded.updated_at returning id,user_id,platform,provider,token,endpoint,p256dh,auth,created_at,updated_at`;return rowToNotificationDevice(rows[0]);},
